...
--
-- Phase 03 (Auth + Identity + Profile). Creates the tables the API reads and
-- writes for authentication, identity and tenant membership, with row level
-- security so the database enforces ownership as well as the API.
--
...
-- ── target role requirements (referenced by profiles) ─────────────────────
...
-- A profile row is created by the database, not by client input, so every
-- authenticated user has exactly one profile.
...
-- ── tenant helpers ────────────────────────────────────────────────────────
-- SECURITY DEFINER so a policy on a membership table can call them without
...
-- profiles: users see and edit only their own row. Inserts come from the
-- signup trigger (SECURITY DEFINER) or from the user themselves.
...
-- colleges: a college is visible when verified, or to its own members.
...
-- organization memberships: own rows, plus colleagues inside the same tenant.
...
-- student ↔ college memberships: the student, and staff of that college.
...
-- Membership rows are provisioned by operators/maintenance jobs, never by the
-- client: no insert/update/delete policy is granted to `authenticated`.
