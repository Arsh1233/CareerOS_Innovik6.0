...
export default function ResumePage() {
  ...
  const { accessToken, profile, refreshUser } = useAuth();
  ...
  async function loadLatest() {
    ...
    setLoading(true);
    try {
      const latest = await resumesApi.getLatestResume(accessToken);
  ...
  async function handleFile(file: File | undefined) {
    ...
    try {
      ...
      const targetRole = profile?.target_role_name ?? undefined;
      const res = await resumesApi.uploadResume(accessToken, file, targetRole);
      setResumeData(res);
      // Refresh user so skill evidence is synced across pages
      refreshUser().catch(() => {});

      if (res.pipeline.analysis_status === "ok") {
        success(`"${file.name}" analysed successfully. Skills synced.`);
  ...
  async function handleRemove() {
    if (!resumeData || !accessToken) return;
    try {
      await resumesApi.deleteResume(accessToken, resumeData.resume_id);
      setResumeData(null);
