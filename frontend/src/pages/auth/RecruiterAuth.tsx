  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const outcome = await signUp({
        email: email.trim(),
        password,
        full_name: fullName.trim() || company.trim() || email.trim().split("@")[0],
        role: "recruiter",
      });
      ...
      success("Account created. Let's set up your organisation.");
        ...
        <div className="w-full max-w-[380px]">
          ...
          <div className="bg-white rounded-2xl border border-[#E4E7EC] shadow-[0_2px_24px_rgba(0,0,0,0.06)] p-7">
            ...
            <form onSubmit={handleSubmit} className={`space-y-3.5 ${transitioning ? "form-exit" : "form-enter"}`}>
              ...
              {notice && (
                <p className="text-[12px] text-[#8B7CFF] bg-[#8B7CFF]/6 border border-[#8B7CFF]/20 rounded-lg px-3 py-2">
                  {notice}
                </p>
              )}
              {redirectRole && (
